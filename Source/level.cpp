...
void Level::Generate( )
{
        ...
        for ( UInt y = 1; y < _h - 1; y++ )
        {
            for ( UInt x = 1; x < _w - 1; x++ )
            {
                ...
                for ( int i = -1; i <= 1; i++ )
                {
                    for ( int j = -1; j <= 1; j++ )
                    {
                        if ( _map[ y + i ][ x + j ].type != MAP_FLOOR )
                        { adjcount_r1++; }
                    }
                }
    ...
    for ( UInt i = 0; i < LEVEL_HEIGHT / 2; i++ )
    {
        ...
        if ( _map[ y ][ x ].type == MAP_FLOOR )
        { _map[ y ][ x ].items.push_back( Item::Generate( )); }
    }
}
